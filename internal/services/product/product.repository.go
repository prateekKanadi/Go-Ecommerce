...
func (repo *ProductRepository) getVariantProduct(variantID string, currency string) (*VariantProduct, error) {
	...
	// Query to fetch the variant product and its price for the specified currency
	query := `
	SELECT 
		v.variantId, 
		v.variantName,
	...
	WHERE 
		v.variantId = ? AND pp.currencyCode = ?
`

	// Fetch the product data for the selected product and currency
	row := repo.db.QueryRow(query, variantID, currency)
	...
	// Handle different error scenarios
	if err == sql.ErrNoRows {
		// No product found for the given variantID and currency
		log.Printf("No variant found for variantID: %s and currency: %s", variantID, currency)
		return nil, nil
	} else if err != nil {
		// Log the error if something else went wrong
		log.Printf("Error fetching variant details: %v", err)
		return nil, fmt.Errorf("error fetching variant details: %v", err)
	}

	// After scanning the price and currencyCode, append the price to the Prices slice
	variantProduct.Prices = append(variantProduct.Prices, price)

	// Log the successful retrieval of the variant product
	log.Printf("Variant retrieved successfully: %+v", variantProduct)
...
func (repo *ProductRepository) getAllVariantProductsByProductID(variantProduct *VariantProduct, currency string) ([]VariantProduct, error) {
	// Ensure the currency is one of the valid options
	if currency != "USD" && currency != "EUR" && currency != "GBP" {
		return nil, fmt.Errorf("invalid currency: %s", currency)
	}
	...
	JOIN
		productPrices pp ON p.productId = pp.productId
	WHERE
		p.productId = ? AND v.variantId != ? AND pp.currencyCode = ?
`

	results, err := repo.db.Query(query, variantProduct.ProductID, variantProduct.VariantID, currency)
	if err != nil {
		log.Println(err.Error())
		return nil, err
	}
